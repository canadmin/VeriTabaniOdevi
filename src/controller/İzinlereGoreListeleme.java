 ...
 * @author can yard
 */
public class İzinlereGoreListeleme {
...
PreparedStatement ps;
                             try {
                                 String sorgu="select * from izinliler where tc_no=? and izin_turu=?";

                                       Connection con=bt.getConnection();
                                       stmt=con.createStatement();
                                     ps = con.prepareStatement(sorgu);
                                      ps.setString(1, YoneticiFormu.aranacakTc);
                                      ps.setString(2, YoneticiFormu.izinturu);
                                        ResultSet rs;
                                       rs=ps.executeQuery();
